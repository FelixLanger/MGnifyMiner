...
def create_parser():
    ...
    phmmer_parser = subparsers.add_parser(
        "phmmer", help="make phmmer search")
    phmmer_parser.add_argument('--query',
                               '-q',
                               type=Path,
                               help='fasta file with query sequence(s)')
    phmmer_parser.add_argument('--target',
                               '-t',
                               type=Path,
                               help='target sequence database to search against')
    phmmer_parser.add_argument('--output',
                               '-o',
                               type=Path,
                               help='output path')
    ...
    filter_parser.add_argument('--coverage',
                               #'-c',
                               type=str,
                               required=False,
                               metavar='[0-100]',
    ...
    filter_parser.add_argument('--eval',
                               #'-e',
                               type=float,
                               required=False,
                               metavar='0.001',
                               help='Filter by e-value. Threshold for highest e-value displayed')
    filter_parser.add_argument('--sort',
                               required=False,
                               nargs='+',
    ...
    residue_parser.add_argument('--input',
                               #'-i',
                               type=Path,
                               required=True,
                               metavar='path/to/filter_output.csv',
                               help='Path to sequence search output file')
    residue_parser.add_argument('--residue',
                               '-r',
                               nargs='+',
                               action='append',
    ...
    residue_checker_parser = subparsers.add_parser("residue_check", help="check which aminoacids can be found on"
                                                                         " the target sequences corresponding to a"
                                                                         " residue in the query")
    residue_checker_parser.add_argument('--residue',
                               type=int,
                               metavar='155',
                               help='Coordinate of the residue in the query sequence')
    residue_checker_parser.add_argument('--input',
                               #'-i',
                               type=Path,
                               required=True,
                               metavar='path/to/filter_output.csv',
                               help='Path to sequence search output file')
    residue_checker_parser.add_argument('--plotwidth',
                               type=int,
                               metavar='100',
                               required=False,
                               default=85,
...
def phmmer(args):
    ...
    targetDB.phmmer(args.query, output=args.output, tblout=tbl, domtblout=dom_tbl, alignment=alignment, notextw=True)
...
def filter(args):
    ...
    if args.coverage:
        ...
        dom_tbl = dom_tbl[dom_tbl['coverage_query'].between(coverage_range[0], coverage_range[1])]

    if args.sort:
        map = {'eval' : 'score', 'coverage' : 'coverage_query', "similarity" : "similarity", "identity" : "identity"}
        if all(filters in map.keys() for filters in args.sort):
            columns = [map[v] for v in args.sort]
            columns.extend(["target_name", "ndom"])     #additionally sort by name and ndom to keep entries from one
            orientation = [False for column in columns] #protein together, ndom sort needs to be ascending to keep
...
def overlapping_targets(args, results_table):
    ...
    selection =  results_table.loc[(results_table["hmm_from"]<=residue_coordinate) &
                                   (results_table["hmm_to"]>=residue_coordinate),
                                   ["target_name", "ali_from", "ali_to"]].to_string(header=False,
                                                                                    index=False,
...
def parse_domtable(file):
    ...
    column_names = ['target_name', 'target_accession', 'tlen', 'query_name', 'query_accession', 'qlen', 'e-value',
                    'score', 'bias', 'ndom', 'ndom_of', 'c-value', 'i-value', 'dom_score', 'dom_bias', 'hmm_from',
                    'hmm_to', 'ali_from', 'ali_to', 'env_from', 'env_to', 'acc', 'description']

    convert_dict = {'target_name' : str, 'target_accession' : str, 'tlen' : int, 'query_name' : str,
                    'query_accession' : str, 'qlen' : int, 'e-value' : float,
                    'score' : float, 'bias': float, 'ndom' : int, 'ndom_of' : int, 'c-value' : float, 'i-value' : float,
                    'dom_score' : float, 'dom_bias' : float, 'hmm_from' : int,
                    'hmm_to' : int, 'ali_from' : int, 'ali_to' : int, 'env_from' : int, 'env_to' : int, 'acc' : float,
                    'description' : str}
...
def get_alignment_consensus(file):
    ...
    for alignment in alignments(file):
        ...
        # Solution: count characters until you hit the first whitespace after hitting letters. Doing it once to get lenght
        # until the name. doing it twice to get characters until the end of the start coordinates.
        ...
        alignments_dict[key] = {"consensus" : consensus,
...
def add_sim_ident(df, alignment_dict):
    df["similarity"] = df["target_name"] + "-" + df["ali_from"].astype(str) + "-" + df["ali_to"].astype(str)
...
def plot_residue_histogram(args):
    ...
    for key, alignment in alignment_dict.items():
        if int(alignment["query_start"]) <= residue and int(alignment["query_end"]) >= residue:
