...
pt = proteinTable("/home/felix/PycharmProjects/MGnifyMiner/playground/miniset.csv")
query = Path("/home/felix/PycharmProjects/MGnifyMiner/playground/query.fa")
tree_out = Path("/home/felix/PycharmProjects/MGnifyMiner/playground/tree.out")
...
app.layout = html.Div(
    [
        html.H1(children="MGnify Miner"),
        html.Div(
            """
    Explore MGnify protein search results
    """
        ),
        html.Br(),
        dbc.Card(
                ...
                [
                        ...
                        [
                            dbc.Col(
                                [
                                    html.Div(
                                        [
                                            dcc.Dropdown(
                                                pt.df.columns, "tlen", id="xaxis-column"
                                            )
                                        ],
                                        style={
                                            "width": "48%",
                            ...
                            dbc.Col(
                                dbc.Tabs(
                                    [
                                        dbc.Tab(
                                            figure("protein_distribution"),
                                            label="Protein Metadata",
                                        ),
                                        dbc.Tab(figure("biome_dist"), label="Biome"),
                                    ]
                                ),
                                width=5,
                            ),
                    ...
                    dbc.Row(
                        [
                            ...
                            html.Div(
                                "Select proteins in scatter plot and press the button"
                                " to start the pylogenetic tree building"
                            ),
                            html.Br(),
                            html.Div(
                                [
                                    dbc.Button(
                                        id="button_id",
                                        children="Create phylogenetic tree",
                                    ),
                                    dbc.Button(
                                        id="cancel_button_id",
                                        children="Cancel tree creation",
                                    ),
                                ],
                                className="d-grid gap-2 col-6 mx-auto",
                            ),
                            dbc.Progress(
                                id="progress_bar", animated=True, striped=True
                            ),
                            html.Div(
                                [figure("tree")],
                                id="tree-field",
...
@app.callback(
    Output("protein_distribution", "figure"), Input("stats-scatter", "selectedData")
)
def update_pie(selectedData):
    if selectedData:
        indices = [e["pointIndex"] for e in selectedData["points"]]
        dff = pt.df.loc[indices]
    else:
        dff = pt.df
    fig = piecharts(dff)
    return fig
...
@app.callback(Output("biome_dist", "figure"), Input("stats-scatter", "selectedData"))
def update_biome(selectedData):
    if selectedData:
        indices = [e["pointIndex"] for e in selectedData["points"]]
        dff = pt.df.loc[indices]
    else:
...
@app.long_callback(
    ...
    inputs=[Input("button_id", "n_clicks"), State("stats-scatter", "selectedData")],
    running=[
...
def build_tree(set_progress, n_clicks, selectedData):
    i = 0
    total = 5
    ...
    i += 1
    set_progress((str(i), str(total)))
    tree = treebuilder(dff, query, tree_out)
    i += 1
    set_progress((str(i), str(total)))
    tree.make_alignment()
    i += 1
...
class treebuilder:
    ...
    def make_alignment(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.fetcher.run(
                "/home/felix/PycharmProjects/MGnifyMiner/playground/minidb.fa",
                keyfile,
                sequences,
                args=["-Cf"],
            )
